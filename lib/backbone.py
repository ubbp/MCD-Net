...
import numpy as np
from .adaptive_deep_fusion_module import ADF
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, fv1:NestedTensor):
        x = fv1.tensors
        mask = fv1.mask
        assert mask is not None
        not_mask = ~mask
...
class MultiModalSwinTransformer(nn.Module):
    def __init__(self,
        ...
        self.sam_model = {}
        self.sam_model['efficientsam_s'] = build_efficient_sam_vits().to('cuda')
        for model_name, model in self.sam_model.items():
            model.eval()  
        ...
        self.adf = ADF(dim=sum(num_features), num_blocks=num_adf)
        self._freeze_stages()
    ...
    def forward(self, x, l, l_mask, t, t_mask, p, p_mask):
        ...
        x_normalized = (x_resized - x_min) / (x_max - x_min)
        with torch.no_grad():
            for model_name, model in self.sam_model.items():
                sam_features, sam_features_list = model.image_encoder(x_normalized)  # (B, 256, H/4, W/4)
                
                multi_samfeature_list = [sam_features_list[0],sam_features_list[1],sam_features_list[2],sam_features_list[4]]
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww, l, l_mask, t, t_mask, p, p_mask,  sam_features, False)
        ...
        outs = self.adf(outs, multi_samfeature_list, l, l_mask)
        return tuple(outs)
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.conv_layer = nn.Conv2d(in_channels=256, out_channels=dim, kernel_size=1)
        ...
        self.context_pixel_attn = PWAM(dim, 
                           dim,  
                           768, 
                           dim,  
                           dim,  
        ...
        self.object_position_align_blk = OPAB(dim,
                                  dim,
                                  768,
                                  dim,
                                  dim,
    ...
    def simple_dct(self, x):
       
        return torch.fft.fft(x, dim=1).real  

    def split_frequency(self, feat, ratio=0.5):
        """
        feat: [B, N, C]
        """
        C = feat.shape[2]
        split_point = int(C * ratio)
        low_freq = feat[:, :, :split_point]  
        high_freq = feat[:, :, split_point:]  
        return low_freq, high_freq

    def plot_spectrum_map(feat, H, W, title='Frequency Spectrum'):
        """
        feat: (B, HW, C) feature after DCT
        """
        B, N, C = feat.shape
        feat_img = feat[0].view(H, W, C).mean(dim=-1).cpu().numpy() 

        plt.figure(figsize=(6, 6))
        plt.imshow(np.log1p(np.abs(feat_img)), cmap='jet')  
        plt.title(title)
        plt.axis('off')
        plt.show()

    def forward(self, x, H, W, l, l_mask, t, t_mask, p, p_mask, this_input_sam_features, sam_flag=False):
        """ Forward function.
        ...
        if sam_flag:
            this_input_sam_features = self.conv_layer(this_input_sam_features)
            sam_features_resize = F.interpolate(this_input_sam_features, size=(H, W), mode='bilinear', align_corners=False)
            sam_features_resize = sam_features_resize.flatten(2).permute(0, 2, 1) 

            x = x + sam_features_resize

        out = []
        x_reshape = x.permute(0,2,1).view(x.shape[0], x.shape[2], H, W)

        x_size = x_reshape.size()
        for i, ps in enumerate(self.psizes):
            px = self.pyramids[i](x_reshape)
            px = px.flatten(2).permute(0,2,1)
            
           
           
            freq_feat = self.simple_dct(px)
            low_freq, high_freq = self.split_frequency(freq_feat, ratio=0.5)

           
            l_ = self.l_proj(l.permute(0,2,1)).permute(0,2,1)  # [B, 32, 20]
            freq_weight = self.freq_gate(l_)  # [B,2]

            
            low_weight = freq_weight[:,0].unsqueeze(-1).unsqueeze(-1)  # [B,1,1]
            high_weight = freq_weight[:,1].unsqueeze(-1).unsqueeze(-1) # [B,1,1]

            
            low_freq = low_freq * low_weight
            high_freq = high_freq * high_weight

            
            low_out = self.csa[i](low_freq, l, l_mask, 'low')    
            high_out = self.csa[i](high_freq, l, l_mask, 'high')   

            
            px_residual = torch.cat([low_out, high_out], dim=-1)
            px_residual = px_residual.permute(0,2,1).view(x.shape[0], self.reduction_dim , ps, ps)
            out.append(F.interpolate(px_residual, x_size[2:], mode='bilinear', align_corners=True).flatten(2).permute(0,2,1)) 

            
            low_out_align = self.fsa[i](low_freq, ps, ps, t, t_mask, p, p_mask, 'low')   
            high_out_align = self.fsa[i](high_freq, ps, ps, t, t_mask, p, p_mask, 'high')  

            
            px_align = torch.cat([low_out_align, high_out_align], dim=-1)
            px_align = px_align.permute(0,2,1).view(x.shape[0], self.reduction_dim , ps, ps)
            out.append(F.interpolate(px_align, x_size[2:], mode='bilinear', align_corners=True).flatten(2).permute(0,2,1))
            # ----------------------------------- #
...
class VTA(nn.Module):
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        ...
        self.image_lang_att = FASpatialLanguageAttention(v_in_channels,  # v_in
                                                            l_in_channels,  # l_in
                                                            key_channels,  # key
                                                            value_channels,  # value
                                                            out_channels=value_channels,  # out
    ...
    def forward(self, x, l, l_mask, freq_type):
        # input x shape: (B, H*W, dim)
        vis = self.vis_project(x.permute(0, 2, 1))  # (B, dim, H*W)

        lang = self.image_lang_att(x, l, l_mask, freq_type)  # (B, H*W, dim)
...
class OPAB(nn.Module):
    ...
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        ...
        self.go_cross_attn = SpatialImageLanguageAttention(v_in_channels,  # v_in
                                                            l_in_channels,  # l_in
                                                            key_channels,  # key
                                                            value_channels,  # value
                                                            out_channels=value_channels,  # out
        ...
        self.sp_cross_attn = SpatialImageLanguageAttention(v_in_channels,  # v_in
                                                            l_in_channels,  # l_in
                                                            key_channels,  # key
                                                            value_channels,  # value
                                                            out_channels=value_channels,  # out
...
class DVTA(nn.Module):
    ...
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        super(DVTA, self).__init__()

        self.dim = dim

        # # Ground Object Branch
        # Ground Object Cross Attention
        self.go_cross_attn = FASpatialLanguageAttention(v_in_channels,  # v_in
                                                            l_in_channels,  # l_in
                                                            key_channels,  # key
                                                            value_channels,  # value
                                                            out_channels=value_channels,  # out
        ...
        # # Spatial Position Branch
        # Spatial Position Cross Attention
        self.sp_cross_attn = FASpatialLanguageAttention(v_in_channels,  # v_in
                                                            l_in_channels,  # l_in
                                                            key_channels,  # key
                                                            value_channels,  # value
                                                            out_channels=value_channels,  # out
    ...
    def forward(self, x, H, W, t, t_mask, p, p_mask, freq_type):
        B = x.shape[0]

        # Ground object branch
        go = self.go_cross_attn(x, t, t_mask, freq_type)
        go = self.go_tanh_gating(go) * go

        # Spatial position branch
        sp = self.sp_cross_attn(x, p, p_mask, freq_type)
        sp = sp.reshape(B, H, W, self.dim).permute(0, 3, 1, 2)
        avg_sp = torch.mean(sp, dim=1, keepdim=True)
        max_sp, _ = torch.max(sp, dim=1, keepdim=True)
        sp = torch.cat([avg_sp, max_sp], dim=1)
...
class SpatialImageLanguageAttention(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        if x.size(2) == 1:
            x = x.permute(0, 2, 1)
            query = self.f_query2(x).permute(0,2,1)
        else:
...
class FASpatialLanguageAttention(nn.Module):
    def __init__(self, v_in_channels, l_in_channels, key_channels, value_channels, out_channels=None, num_heads=1, freq_embed_dim=16):
        super(FASpatialLanguageAttention, self).__init__()
        self.v_in_channels = v_in_channels + 16
        self.l_in_channels = l_in_channels
        self.out_channels = out_channels
        ...
        # Key & Value from language feature
        self.f_key = nn.Sequential(
            nn.Conv1d(self.l_in_channels, self.key_channels, kernel_size=1, stride=1),
        )
        # Values: language features: (B, l_in_channels, #words)
        ...
        self.freq_embedding = nn.Embedding(2, freq_embed_dim)
    ...
    def forward(self, x, l, l_mask, freq_type):
        ...
        if freq_type == 'low':
            freq_t = torch.zeros(B, HW, dtype=torch.long)
        else:
            freq_t = torch.ones(B, HW, dtype=torch.long)
        freq_t = freq_t.cuda()
        freq_emb = self.freq_embedding(freq_t)  # (B, HW, freq_embed_dim)
        x_aug = torch.cat([x, freq_emb], dim=-1)  # (B, HW, v_in_channels+freq_embed_dim)
        x_aug = x_aug.permute(0, 2, 1)  # (B, key_channels, H*W)
        if x_aug.size(2) == 1:
            x_aug = x_aug.permute(0, 2, 1)
            query = self.f_query2(x_aug).permute(0,2,1)
        else:
            query = self.f_query(x_aug)  # (B, key_channels, H*W) if Conv1D
        query = query.permute(0, 2, 1)  # (B, H*W, key_channels)
